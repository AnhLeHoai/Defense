...
public class LuckyWheel : UICanvas
{
    ...
    private void Update()
    {
        ...
        if (_time >= 1)
        {
            ...
            txtClockFreeSpin.text = Helper.FormatTimeIgnoreSecond(timeSpan.Hours, timeSpan.Minutes, true);
            _time = 0;
        }
    }
    ...
    private void OnCompleteRotate()
    {
        ...
        switch (dataReward.Type)
        {
            ...
            case TypeGift.HAT:
                {
                    if (playerData.GetUnlockSkin(TypeEquipment.HAT, dataReward.IdType))
                    {
                    ...
                    else
                    {
                        ...
                        playerData.SetUnlockSkin(TypeEquipment.HAT, dataReward.IdType);
                    }
                }
                break;
            case TypeGift.SKIN:
                {
                    if (playerData.GetUnlockSkin(TypeEquipment.SKIN, dataReward.IdType))
                    {
                        GameManager.Instance.Profile.AddGold(dataReward.NumberCoinReplace, "lucky_wheel");
                        SoundManager.Instance.PlaySoundReward();
                    ...
                    else
                    {
                        ...
                        playerData.SetUnlockSkin(TypeEquipment.SKIN, dataReward.IdType);
                    }
                }
                break;
            case TypeGift.PET:
                {
                    if (playerData.GetUnlockSkin(TypeEquipment.PET, dataReward.IdType))
                    {
                        GameManager.Instance.Profile.AddGold(dataReward.NumberCoinReplace, "lucky_wheel");
                        SoundManager.Instance.PlaySoundReward();
                    }
                    else
                    {
                        ...
                        GameManager.Instance.UiController.OpenPopupReward(reward, TypeDialogReward.LUCKY_WHEEL);
                        playerData.SetUnlockSkin(TypeEquipment.PET, dataReward.IdType);
    ...
    private IEnumerator ActionWatchVideo()
    {
        ...
        while (_tmp1 < 2)
        {
            ...
            {
                _tmp2 = 0;
                _rewardAdStatus = AdManager.Instance.ShowAdsReward(OnRewardVideo, Helper.video_reward_lucky_wheel, false);
